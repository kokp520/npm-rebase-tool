...
async function checkCommits(master) {
...
async function promptForUpdateMsg(commitMsg) {
...
async function main() {
    ...
    const master = await promptForBranch();
    await checkCommits(master);

    console.log(`WARNING: This script will squash all commits after ${master} branch.`);
    const continueRebase = await promptForContinue();
    if (!continueRebase) {
        process.exit(1);
    }
    ...
    commitMsg = await promptForUpdateMsg(commitMsg);
